 ...
 * *client* helpers (via a lazy `import("ai")`). This SignRequest server uses
 * only the McpAgent *server* path, so that code is never reached. We alias
 * `ai` -> this stub in wrangler.jsonc to keep the unused SDK out of the bundle.
 *
